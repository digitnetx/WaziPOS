...
export const ReceiptForm: React.FC<ReceiptFormProps> = ({ onSubmit, isSubmitting }) => {
  ...
  const handleFormSubmit = (values: z.infer<typeof formSchema>) => {
    try {
      ...
      form.setError('root', { type: 'manual', message: 'Unable to generate this receipt on this POS. Please try again.' });
    }
  };
      ...
      <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-4 md:space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField control={form.control} name="customerName" render={({ field }) => (
            <FormItem className="md:col-span-2"><FormLabel className="text-xs md:text-sm">Payer Name</FormLabel><FormControl><Input placeholder="Enter payer name" {...field} className="h-9 md:h-10 text-sm" /></FormControl><FormMessage /></FormItem>
          )} />
          <FormField control={form.control} name="customerPhone" render={({ field }) => (
            <FormItem><FormLabel className="text-xs md:text-sm">Payer Phone</FormLabel><FormControl><Input placeholder="e.g. 0000000000" {...field} className="h-9 md:h-10 text-sm" /></FormControl><FormMessage /></FormItem>
          )} />
          <FormField control={form.control} name="billItem" render={({ field }) => (
            <FormItem><FormLabel className="text-xs md:text-sm">Bill Item</FormLabel><FormControl><Input placeholder="e.g. Entrance fees..." {...field} className="h-9 md:h-10 text-sm" /></FormControl><FormMessage /></FormItem>
          )} />
          <FormField control={form.control} name="visitorType" render={({ field }) => (
            <FormItem><FormLabel className="text-xs md:text-sm">Visitor Category</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger className="h-9 md:h-10 text-sm"><SelectValue placeholder="Select type" /></SelectTrigger></FormControl><SelectContent><SelectItem value="International Tourist">International Tourist</SelectItem><SelectItem value="Local Resident">Local Resident</SelectItem><SelectItem value="School Group">School Group</SelectItem><SelectItem value="EAC Resident">EAC Resident</SelectItem></SelectContent></Select><FormMessage /></FormItem>
          )} />
        ...
        <div className="pt-2"><Button type="submit" className="w-full gap-2 text-base md:text-lg h-12 md:h-14 font-bold" disabled={isSubmitting}>{isSubmitting ? <Loader2 className="w-5 h-5 animate-spin" /> : <><Save className="w-5 h-5" />Generate Government Bill</>}</Button>{form.formState.errors.root?.message && <p className="mt-2 text-sm text-destructive text-center">{form.formState.errors.root.message}</p>}</div>
      </form>
    </Form>
  );
};
