...
export const ThermalReceiptCompact: React.FC<ThermalReceiptCompactProps> = ({ receipt, className, id, paperWidth = '58mm' }) => {
  ...
  return (
    <div id={id} style={style} className={className}>
      <div style={{ textAlign: 'center', marginBottom: small ? '9px' : '11px' }}>
        <div style={{
          fontSize: small ? '13px' : '13.5px',
          fontWeight: 600,
          lineHeight: 1.08,
          whiteSpace: 'nowrap',
          marginBottom: small ? '9px' : '11px',
        }}>
      ...
      <div>
        {/* Keep BillItem + its complete value on one physical line on 58mm. */}
        <div style={{
          ...row,
          whiteSpace: 'nowrap',
          fontSize: small ? '10px' : '13.5px',
          letterSpacing: small ? '-0.15px' : undefined,
        }}>
          BillItem : {receipt.billItem}
        </div>
        {/* Immediately follows BillItem: no blank line. */}
        <div style={row}>({receipt.currency})</div>
        {/* Immediately follows currency: no blank line. */}
      ...
      <div style={{ marginTop: 0, marginBottom: small ? '12px' : '13px', lineHeight: 1.22 }}>
